...
function clampScroll(){
  var maxScroll=Math.max(0,candles.length-2);
  vp.scrollOffset=Math.max(0,Math.min(maxScroll,vp.scrollOffset));
...
function zoomAtCursor(cursorX,deltaY){
  ...
  vp.scrollOffset=candles.length-newEndIdx;
  clampScroll();
}
...
cv.addEventListener('wheel',function(e){
  ...
  // Cmd/Ctrl+scroll → zoom at cursor (TradingView style)
  if(e.metaKey||e.ctrlKey){
    zoomAtCursor(x,e.deltaY);
    return;
  }
  // Plain scroll / Shift+scroll → horizontal pan
  var delta=e.deltaY||e.deltaX;
  var range=getVisibleRange();
  var candlesDelta=delta/range.step;
  vp.scrollOffset+=candlesDelta;
  clampScroll();
},{passive:false});
...
cv.addEventListener('mousemove',function(e){
  ...
  if(vp.dragMode==='pan'){
    ...
    vp.scrollOffset=vp.dragStartVp.scrollOffset+candlesDelta;
    clampScroll();
